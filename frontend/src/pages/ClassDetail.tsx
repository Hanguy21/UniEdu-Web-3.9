...
function ClassDetail() {
  ...
  // ID nhân sự hiện tại (khi đăng nhập bằng tài khoản teacher)
  const currentUserStaffId = currentUser?.role === 'teacher' ? (currentUser?.linkId || null) : null;
  // Giáo viên chỉ được xem/chỉnh sửa buổi học do mình tạo (teacher_id = mình); admin/accountant/cskh được xem tất cả
  const canEditThisSession = useCallback((session: any) => {
    if (!isAuthenticated) return false;
    if (isAdmin || hasRole('accountant') || hasCskhPrivileges) return true;
    if (!isTutor || !currentUserStaffId) return false;
    const sessionTeacherId = (session as any).teacherId || session.teacher_id;
    return sessionTeacherId === currentUserStaffId;
  }, [isAuthenticated, isAdmin, isTutor, hasRole, hasCskhPrivileges, currentUserStaffId]);
  ...
  return (
    <div className="page-container" style={{ padding: 'var(--spacing-6)' }}>
                    ...
                    {monthSessions.map((session) => {
                      ...
                      return (
                        <tr
                          ...
                          style={{
                            cursor: canEditThisSession(session) ? 'pointer' : 'default',
                            transition: 'all 0.2s ease',
                          ...
                          onClick={(e) => {
                            ...
                            // Open edit modal when clicking on row (chỉ khi được phép: admin/accountant/cskh hoặc giáo viên tạo buổi đó)
                            if (canEditThisSession(session)) {
                              setEditingSession(session);
                              setEditSessionModalOpen(true);
                            } else if (isTutor && !canEditThisSession(session)) {
                              toast.warning('Bạn chỉ có thể xem và chỉnh sửa buổi học do mình tạo');
                            }
                          }}
                          onMouseEnter={(e) => {
      ...
      >
        {id && classData && (
          <AddSessionModal
            key={addSessionModalOpen ? 'open' : 'closed'}
            classId={id}
            classData={classData}
            teachers={classTeachers}
            students={enrolledStudents}
            defaultTeacherId={currentUserStaffId && classTeachers.some((t: any) => t.id === currentUserStaffId) ? currentUserStaffId : undefined}
...
function AddSessionModal({
  ...
  const [teacherId, setTeacherId] = useState<string>(() => {
    if (defaultTeacherId && teachers.some((t: any) => t.id === defaultTeacherId)) return defaultTeacherId;
...
export function EditClassModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Cho phép bỏ trống ô scale và trợ cấp tối đa → lưu là 0
      classDataToSave.scaleAmount = Number(formData.scaleAmount) || 0;
      classDataToSave.maxAllowancePerSession = Number(formData.maxAllowancePerSession) || 0;
